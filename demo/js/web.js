  ...
  const WebBrowser = {
    ...
    render(page) {
      ...
      clip.addEventListener("click", async () => {
        const url = page.url;
        if (!url) { U.toast(U.t("请先输入网址")); return; }
        clip.disabled = true;
        clip.textContent = "⏳";
        U.toast(U.t("正在抓取网页…"));
        try {
          ...
          if (!data || !data.markdown) { U.toast(U.t("抓取失败：无法提取正文")); return; }
          global.App.createClippedNote(data.title || page.title, data.markdown, url);
        } catch (e) { U.toast(U.t("抓取失败：" + (e.message || String(e)))); }
