...
(function (global) {
  ...
  const formatTime = value => {
    if (!value) return "从未";
    try { return new Date(value).toLocaleString("zh-CN"); } catch (error) { return "未知"; }
  };
  ...
  const MemoryPage = {
    ...
    async render() {
      ...
      body.appendChild(U.el("div", "empty-state", "正在加载记忆…"));
    ...
    renderHeader(scroll) {
      ...
      copy.appendChild(U.el("div", "settings-title", "记忆"));
      copy.appendChild(U.el("div", "settings-section-desc", "这里汇总 AI 助手的全部长期记忆。修改后会同步用于近期提示和按需检索。"));
      const add = U.el("button", "db-btn primary", "＋ 添加记忆");
    ...
    renderList(body) {
      ...
      if (count) count.textContent = this.entries.length + " 条长期记忆";
      if (!this.entries.length) {
        body.appendChild(U.el("div", "empty-state", "还没有长期记忆。点击“添加记忆”创建第一条。"));
        return;
      }
    ...
    renderCard(entry) {
      ...
      text.maxLength = 2000;
      text.rows = 4;
      text.setAttribute("aria-label", "记忆正文");

      const footer = U.el("div", "memory-card-footer");
      const meta = U.el("div", "memory-meta", "创建于 " + formatTime(entry.createdAt) + " · 最近访问 " + formatTime(entry.lastAccess));
      const actions = U.el("div", "memory-actions");
      const label = U.el("label", "memory-importance-label", "重要性");
      const importance = U.el("select", "memory-importance");
      importance.setAttribute("aria-label", "记忆重要性");
      for (let value = 1; value <= 5; value++) {
        const option = U.el("option", null, value + (value === 1 ? " · 低" : value === 5 ? " · 高" : ""));
      ...
      const remove = U.el("button", "db-btn danger", "删除");
    ...
    renderError(body, error) {
      ...
      wrap.appendChild(U.el("div", null, "记忆加载失败：" + error.message));
      const retry = U.el("button", "db-btn", "重试");
    ...
    async create() {
      try {
        const result = await request("/api/memory/save", {
          ...
          body: JSON.stringify({ text: "新记忆", importance: 3 }),
        });
        ...
        U.toast(result.indexed ? "已添加记忆" : "已添加记忆；向量索引暂未同步");
      } catch (error) {
        U.toast("添加失败：" + error.message);
      }
    },

    async save(id, text, importance, button) {
      const value = text.value.trim();
      if (!value) { U.toast("记忆正文不能为空"); text.focus(); return; }
      ...
      try {
        ...
        U.toast(result.indexed ? "记忆已保存" : "记忆已保存；向量索引暂未同步");
      } catch (error) {
        U.toast("保存失败：" + error.message);
      } finally {
    ...
    async remove(id) {
      const confirmed = await U.confirmModal("删除记忆", "确定永久删除这条长期记忆吗？此操作不可撤销。", "删除");
      if (!confirmed) return;
      try {
